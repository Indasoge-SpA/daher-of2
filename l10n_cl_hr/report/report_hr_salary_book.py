...
class report_hr_salary_employee_bymonth(models.AbstractModel):
    ...
    def get_worked_days(self, form, emp_id, emp_salary, mes, ano):
        self.env.cr.execute(
            '''select sum(number_of_days) from hr_payslip_worked_days as wd
left join hr_payslip as r on r.id = wd.payslip_id
left join hr_work_entry_type wet on (wet.id = wd.work_entry_type_id)
where r.employee_id = %s  and (to_char(date_to,'mm')= %s)
and (to_char(date_to,'yyyy')= %s) and (wet.code = %s)
''', (emp_id, mes, ano,'WORK100'))
...
group by r.name, p.date_to''', (cod_id, mes, ano,self.env.user.company_id.id))
...
group by r.name, p.date_to,emp.id''', (emp_id, cod_id, mes, ano,self.env.user.company_id.id))
...
order by r.analytic_account_id, last_name''', (last_month, last_year,self.env.user.company_id.id))
